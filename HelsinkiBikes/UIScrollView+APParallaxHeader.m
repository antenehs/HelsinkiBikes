...
- (void)removeParallaxWithView {
    if(self.parallaxView) {
//        [self.parallaxView.currentSubView removeFromSuperview];
//        self.parallaxView.frame = CGRectMake(0, 0, 0, 0);
    }
}

- (void)addParallaxWithView:(UIView*)view andHeight:(CGFloat)height {
    if(self.parallaxView) {
        self.parallaxView.frame = CGRectMake(self.parallaxView.frame.origin.x, self.parallaxView.frame.origin.y, self.bounds.size.width, height);
        [self.parallaxView.currentSubView removeFromSuperview];
...
- (void)drawRect:(CGRect)rect {
    ...
    //// Rectangle Drawing
    UIBezierPath* rectanglePath = [UIBezierPath bezierPathWithRect: CGRectMake(0, 0, CGRectGetWidth(rect), 0)];
